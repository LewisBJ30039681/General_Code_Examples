...
def collide(p1, p2):
    ...
    if distance < p1.size + p2.size:
        print(f"Collision detected between {p1.size} and {p2.size}")  # Debugging print

        #check if they should combine before bouncing
        if ((p1.size < 20 and p2.size < 20) or (20 <= p1.size < 30 and 20 <= p2.size < 30)):
            p1.combine(p2)
            return
...
class Particle:
    ...
    def combine(self, otherParticle):
        ...
        if dist < self.size + otherParticle.size:
            #check if both particles are either small or medium
            if ((self.size < 20 and otherParticle.size < 20) or (20 <= self.size < 30 and 20 <= otherParticle.size < 30)):
                print(f"Combining particles: {self.size} + {otherParticle.size}")  #debuging

                #work out the new combined size and mass
                total_mass = self.mass + otherParticle.mass
                new_size = self.size + otherParticle.size
                ...
                (self.angle, self.speed) = addVectors(
                    self.angle, self.speed * self.mass / total_mass,
                    otherParticle.angle, otherParticle.speed * otherParticle.mass / total_mass
                )

                #account for collision energy loss
                ...
                #mark the other particle for removal
...
class Environment:
    ...
    def update(self):
        ...
        # call twoparticle functions eg collisions, attraction, combine
        ...
        # remove merged particles that were marked for deletion
